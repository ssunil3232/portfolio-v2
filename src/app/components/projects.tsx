    ...
    <div className='flex flex-col min-h-screen w-full'>
        ...
        {projects.map((project, index) => (
            <Link href={project.link} key={index}>
              <div className="block">
                <div  className='card-content w-full pb-8' style={{background: colors[index]}}>
                    {/* <Image src={project.logo} alt={`${project.id}-Logo`} width={150} height={150} layout="responsive" className="rounded-lg px-16" /> */}
                ...
                {project.tools.map((point, idx) => (
                  <div
                    key={idx}
                    className="py-1 px-3 bg-[#ECECEC] rounded-3xl mb-3 "
                  >
                    {point}
                  </div>
                ))}
